...
module _ where
  ...
  Data.Product.proj₁ (UFD.factor (Field-is-UFD {R} FR) ((q , q∉) , qprf)) = S (S Z)
  Data.Product.proj₁ (Data.Product.proj₂ (UFD.factor (Field-is-UFD {R} FR) ((q , q∉) , qprf))) = {!p!}
    where
      open Field FR
      +-inj   : ∀ {x} → x + x ≡ x → x ≡ z
      +-inj {x} x+x≡x = sym (   sym ((cong (λ y → y + (- x)) x+x≡x) ⋆ -inv-+) ⋆ +-assoc ⋆ (cong (λ y → x + y) -inv-+) ⋆ z-id ) 
      ...
      qwwq : ∀ {(y , y∉) (x , x∉) : (R \-- R^×)} → (R \-- R^×)
      proj₁ (qwwq {y , y∉} {x , x∉}) = y ∙ x
      proj₂ (qwwq {y , y∉} {x , x∉}) ((.((FR Field.∙ y) x) , w , w∙r≡e) , refl) = x∉ ( (x , ((w ∙ y) , (∙-assoc ⋆ w∙r≡e) )) , refl )

      z== : ((x , x≢z) : (R \- z)) →  R^×-set
      z== (x , x≢z) = x , ((intro- (inv (x , x≢z))) , inv-inv-∙ )

      frrr : ∀ {((y , y∉R^×) , b)  ((x , x∉R^×) , d) : (R \-- R^×) \- qz } → y ∙ x ≢ e
      frrr {((y , y∉R^×) , b)} {((x , x∉R^×) , d)} y∙x≡e = x∉R^× ( (x , (y , y∙x≡e)) , refl)

      g : ∀ {((y , y∉R^×) , b)  ((x , x∉R^×) , d) : (R \-- R^×) \- qz } → y ∙ x ≢ q
      g {((y , y∉R^×) , b)} {((x , x∉R^×) , d)} y∙x≡q = {!!}
    
      p : Fin (S (S Z)) → Irreduci
      p (Sf Zf) = (e , frrr)
      p Zf      = (q , g)
      
  Data.Product.proj₁ (Data.Product.proj₂ (Data.Product.proj₂ (UFD.factor (Field-is-UFD {R} FR) ((q , q∉) , qprf)))) = {!!}
  Data.Product.proj₂ (Data.Product.proj₂ (Data.Product.proj₂ (UFD.factor (Field-is-UFD {R} FR) ((q , q∉) , qprf)))) = {!!}
